...
# Install Claude Code CLI and prevent default config creation
        ...
        try:
            ...
            try:
                ...
                if claude_json_path.exists():
                    # Copy ~/.claude.json to container via volume mount to a temporary location
                    validated_path = self._validate_mount_path(claude_json_path, "Claude JSON config")
                    docker_cmd.extend(["-v", f"{validated_path}:/tmp/claude_credentials.json:ro"])
                    print("🔐 Copying ~/.claude.json for native Claude Code authentication")
