...
def test_safety_checks():
    ...
    for env in safe_envs:
        assert cm._is_safe_env_var(env) == True
    ...
    for env in unsafe_envs:
        assert cm._is_safe_env_var(env) == False
    ...
    for inp in safe_inputs:
        assert cm._is_safe_input(inp) == True
    ...
    for inp in unsafe_inputs:
        assert cm._is_safe_input(inp) == False
...
def test_temp_credential_files():
    ...
    print(f"✅ Temp credential file created with correct permissions")
    ...
    try:
        ...
        try:
            ...
            try:
                ...
                return confirm("Did the container setup look correct in the output above?")
            ...
            print(f"⚠️  Container setup test completed with expected error: {type(e).__name__}")
            return True
...
def test_volume_validation():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        valid_volumes = [
            f"{test_dir}:/workspace/data:ro",
            f"{temp_dir}:/tmp/shared:rw"
        ]
