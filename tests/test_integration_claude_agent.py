...
def test_status_commands():
    ...
    status_commands = [
        ...
        (["python3", "toren.py", "status", "--filter", "completed"], "Filter completed", 15),
        (["python3", "toren.py", "status", "--filter", "running"], "Filter running", 15),
    ]
...
def test_run_command_interactive():
    ...
    try:
        ...
        if not confirm("This will create a test job that writes a file and commits it. Continue?"):
        ...
        # Check Docker
        docker_check, _, _ = run_command(["docker", "--version"], "Docker version check")
        if not docker_check:
        ...
        run_cmd = [
            "python3", "toren.py", "run",
            "--spec", spec_file,
            "--base-image", "alpine:latest",
            "--branch", f"test-integration-{int(time.time())}",
        ...
        success, stdout, stderr = run_command(run_cmd, "Full integration test", timeout=300)
        ...
        return confirm("Did the integration test complete successfully? (Check for test_output.txt file and git commit)")
